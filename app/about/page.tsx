    ...
    <div className="min-h-screen flex flex-col items-center p-4 md:p-8 checker-bg">
      <main className="w-full max-w-3xl flex flex-col items-center">
        ...
        <div className="text-center mb-10">
          ...
          <div className="flex items-center justify-center gap-3 text-xl animate-[pulse-glow_2s_ease-in-out_infinite]">
        ...
        {/* Bio Section */}
        <section className="neon-box rounded-lg p-6 md:p-8 bg-black/50 backdrop-blur-sm mb-8 tilt-card w-full">
          <h2 className="text-3xl impact-font text-[#ff10f0] mb-4 glitch-text text-center">
            About Me
          </h2>
        ...
        {/* Interests */}
        <section className="neon-box rounded-lg p-6 md:p-8 bg-black/50 backdrop-blur-sm mb-8 tilt-card w-full">
          <h2 className="text-3xl impact-font text-[#00ffff] mb-6 glitch-text text-center">
            My Interests
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div
                ...
                style={{ borderColor: interest.color, animationDelay: `${i * 0.1}s` }}
              >
                <div className="text-3xl mb-2 animate-[float_3s_ease-in-out_infinite]" style={{ animationDelay: `${i * 0.2}s` }}>
                  {interest.emoji}
                </div>
        ...
        <section className="neon-box rounded-lg p-6 md:p-8 bg-black/50 backdrop-blur-sm mb-8 w-full">
          <h2 className="text-3xl impact-font text-[#39ff14] mb-4 glitch-text text-center">
            Gaming
          </h2>
          ...
          <p className="text-sm text-gray-400 mb-8 italic text-center">
          ...
          {Object.entries(favoriteGames).map(([category, games]) => (
            <div key={category} className="mb-8">
              <h3 className="text-2xl impact-font text-[#ff10f0] mb-4 flex items-center justify-center gap-2">
                <span className="animate-[spin-slow_3s_linear_infinite] text-[#fff700]">⭐</span>
                {category}
                <span className="animate-[spin-slow_3s_linear_infinite_reverse] text-[#fff700]">⭐</span>
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {games.map((game, i) => (
                  <div
                    ...
                    className="tilt-card dashed-border rounded-lg p-4 bg-black/60 text-center"
                    style={{ animationDelay: `${i * 0.15}s` }}
                  >
                    <h4 className="text-lg font-bold text-[#00ffff] mb-1">{game.name}</h4>
                    <span className="pixel-font text-[10px] text-[#39ff14]">
                      {game.year}
                    </span>
                    <p className="text-xs text-gray-400 leading-relaxed mt-2">{game.desc}</p>
                  </div>
                ))}
              </div>
            </div>
